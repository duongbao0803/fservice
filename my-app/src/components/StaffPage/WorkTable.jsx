    ...
    const fetchAllApartmentsAndCreateRows = async () => {
      ...
      const rowsWithData = await Promise.all(staffData?.map(async (staff, index) => {
        ...
        return {
          id: staff.id,
          stt: index + 1,
          apartment: `${apartmentInfo?.type.building.name} - ${apartmentInfo?.roomNo} ` || "N/A", // Assuming you want the apartment name
    ...
    <div className="data-table mt-5" >
      <div>
        <div>
          ...
          {/* Modal */}
          {isModalOpen && (
            <Modal staffData={staffData} isOpen={isModalOpen} service={selectedService} onClose={() => setModalOpen(false)} />
          )}
        </div>
      </div>
    </div>
